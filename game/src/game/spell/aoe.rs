...
impl AoeSpell {
    ...
    pub fn next_tick(&mut self) -> Option<Vec<(u16, u16)>> {
        ...
        for i in -current_radius..=current_radius {
            ...
            if j_abs != 0 {
                // Avoid duplicating the center line if j_abs is 0
                affected_cells.push((
                    (self.caster_row as i16 + i) as u16,
                    (self.caster_col as i16 - j_abs) as u16,
                ));
...
mod tests {
    ...
    fn test_aoe_spell_next_tick_timing_and_completion() {
        ...
        assert!(
            spell.next_tick().is_none(),
            "next_tick should return None immediately after creation"
        );
        ...
        assert!(
            first_tick_cells.is_some(),
            "First tick should occur after interval"
        );
        assert_eq!(
            spell.current_tick_level, 1,
            "current_tick_level should be 1 after first tick"
        );

        // Second tick - not enough time passed (because last_damage_time was just reset)
        assert!(
            spell.next_tick().is_none(),
            "next_tick should return None if not enough time passed"
        );
        ...
        assert!(
            second_tick_cells.is_some(),
            "Second tick should occur after interval"
        );
        assert_eq!(
            spell.current_tick_level, 2,
            "current_tick_level should be 2 after second tick"
        );
        ...
        assert!(
            third_tick_cells.is_some(),
            "Third tick should occur after interval"
        );
        assert_eq!(
            spell.current_tick_level, 3,
            "current_tick_level should be 3 after third tick"
        );
        ...
        assert!(
            spell.next_tick().is_none(),
            "Spell should be finished after max_radius ticks"
        );
        assert_eq!(
            spell.current_tick_level, 4,
            "current_tick_level should be > max_radius when finished"
        );
    }
    ...
    fn test_aoe_spell_next_tick_affected_cells() {
        ...
        let mut expected_r1 = vec![
            ...
            (11, 10),
            (10, 9),
            (9, 10), // Manhattan distance 1 from (10,10)
        ];
        ...
        let mut expected_r2 = vec![
            ...
            (11, 11),
            (12, 10),
            (11, 9), // Manhattan distance 2 from (10,10)
            (10, 8),
            (9, 9),
            (8, 10),
            (9, 11),
        ...
        let mut expected_r3 = vec![
            ...
            (11, 12),
            (12, 11),
            (13, 10), // Manhattan distance 3 from (10,10)
            (12, 9),
            (11, 8),
            (10, 7),
            (9, 8),
        ...
        assert!(
            spell.next_tick().is_none(),
            "Should return None after max_radius ticks"
        );
    }
}

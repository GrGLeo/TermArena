...
3.  **Go Server to Message Service:** The Go server communicates with the Message service via gRPC to handle real-time messaging between clients.
4.  **Go Server Spawns Rust Game Server:** When a game room is created, the Go server spawns a new Rust game server process.
5.  **Client to Rust Game Server:** The client connects directly to the Rust game server to play the game.
6.  **Client to Message Service:** The client connects directly to the Message service for real-time messaging.
7.  **Rust Game Server to Client:** The Rust game server sends real-time game state updates to the client.
8.  **Message Service to Client:** The Message service delivers real-time messages to connected clients.

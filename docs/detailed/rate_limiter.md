...
- Watch for unusual spikes that may indicate attacks
...
1. **Rate Limits Too Restrictive**
   - Increase capacity or refill rates in configuration
   - Monitor legitimate user behavior patterns

2. **Memory Usage High**
   - Check cleanup routine is running
...
- Check `GetLimiterCount()` for active limiter count
- Monitor token bucket states during testing

## Future Enhancements

Potential improvements for the rate limiter:

1. **Distributed Rate Limiting**: Redis-based for multi-server deployments
2. **Dynamic Limits**: Adjust limits based on server load
3. **Request Type Expansion**: Add limits for additional operations

...
- **`model/`:** Contains the different models that are used to represent the application's state, including `messaging.go` for the messaging interface.
- **`communication/`:** Handles communication with the server, including sending and receiving network packets and messaging.
